...
public class ClipboardMonitorService: ObservableObject {
    ...
    private func isContentIgnored(pasteboard: NSPasteboard) -> Bool {
        // 1. Check Pasteboard Types (Concealed/Transient)
        // print("Pasteboard types: \(pasteboard.types?.map { $0.rawValue } ?? [])")
        if let types = pasteboard.types {
            for ignoredType in Self.ignoredPasteboardTypes {
                if types.contains(ignoredType) {
                    return true
                }
            }
        }
        ...
        if let sourceApp = NSWorkspace.shared.frontmostApplication?.bundleIdentifier {
            ...
            if Self.ignoredBundleIds.contains(sourceApp) {
                print("⛔️ Ignored content from password manager: \(sourceApp)")
                return true
            }
        }
    ...
    private func processClipboardContent() {
        ...
        // Check for exclusions (Password Managers, Concealed types)
        if isContentIgnored(pasteboard: pasteboard) {
            print("Clipboard content ignored")
            return
        }
